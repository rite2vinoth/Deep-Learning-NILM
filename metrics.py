...
def recall_precision_accuracy_f1_bacc(pred, ground):
    ...
    if sum_samples == 0:
        ...
        res_bacc = bacc(tp, tn, fp, fn)

        return (res_recall, res_precision, res_accuracy, res_f1, res_bacc)
...
def rmse(pred, ground):
    ground_truth_df = pd.DataFrame(next(ground.power_series()))
    predicted_df = pd.DataFrame(next(pred.power_series()))
    rmse = compute_rmse(ground_truth_df,predicted_df,pretty=True)
    return rmse
...
def bacc(tp, tn, fp, fn):
    return (0.5*((tp/float(tp+fn))+(tn/float(tn+fp))))