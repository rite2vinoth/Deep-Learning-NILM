...
from keras.losses import BinaryCrossentropy, KLDivergence, kl_divergence
from keras.layers import Dense, Flatten, Conv1D, Reshape, Dropout, Input, BatchNormalization
...
class HYBDisaggregator(Disaggregator):
    """
    Attributes
    ----------
    ...
    def train_on_chunk(self, mainchunk, meterchunk, epochs, batch_size):
        ...
        cbacks = [ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=3, min_lr=1e-5)
        ]

        history = self.model.fit(
            X_batch, Y_batch, batch_size=batch_size, epochs=epochs, callbacks=cbacks, validation_split=0.1, shuffle=True
        )
    ...
    def train_across_buildings(
        self, mainlist, meterlist, epochs=1, batch_size=64, **load_kwargs
    ):
        assert len(mainlist) == len(
            meterlist
        ), "Number of main and meter channels should be equal"
    ...
    def train_across_buildings_chunk(self, mainchunks, meterchunks, epochs, batch_size):
        ...
        for e in range(epochs):
            ...
            for bi, b in enumerate(batch_indexes):
                ...
                cbacks = [ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=3, min_lr=1e-8)]

                history = self.model.fit(X_batch, Y_batch, batch_size=batch_size, epochs=epochs, callbacks=cbacks, validation_split=0.1, shuffle=True)
                plt.plot(history.history['loss'], label='train')
                plt.plot(history.history['val_loss'], label='validation')
                plt.legend()
                plt.show()
        ...
        **load_kwargs : key word arguments
        ...
        for chunk in mains.power_series(**load_kwargs):
            if len(chunk) < self.MIN_CHUNK_LENGTH:
                continue
            print("Result chunk: {}".format(len(chunk)))
    ...
    def create_vae(self,sequence_len):
        """ Not being used. Unable to train combined VAE GAN Model"""
        vae = Sequential()

        # 1D Conv
        vae.add(
            Conv1D(8, 4, activation="linear", input_shape=(sequence_len, 1), padding="same", strides=1,)
        )
        vae.add(Flatten())

        # Fully Connected Layers
        vae.add(Dropout(0.2))
        vae.add(Dense((sequence_len) * 8, activation="relu"))

        vae.add(Dropout(0.2))
        vae.add(Dense(128, activation="relu",name='latent'))

        vae.add(Dropout(0.2))
        vae.add(Dense((sequence_len) * 8, activation="relu",activity_regularizer='l1'))

        vae.add(Dropout(0.2))

        # 1D Conv
        vae.add(Reshape(((sequence_len), 8)))
        vae.add(Conv1D(1, 4, activation="linear", padding="same", strides=1))

        vae.compile(loss='mse', optimizer=SGD(learning_rate=0.1, momentum=0.9))
        plot_model(vae, to_file="model.png", show_shapes=True)
        
        return vae

    def create_generator(self, sequence_len):
        """ Returns generator instance
        
        Parameterts
        -----------
        sequence_len = Sequence length from the Disaggregator instance
        ...
        generator.add(Dense(256, input_dim=sequence_len, activation="relu"))
        
        generator.add(Dense(512, activation = "relu"))
        
        generator.add(Dense(1024,activation = "relu"))
        
        generator.add(Dense(sequence_len, activation='tanh'))
        
        generator.compile(loss='mse', optimizer=Adam(learning_rate=0.001))
        
        return generator

    def create_discriminator(self,sequence_len):
        ...
        discriminator.add(Dense(1024, input_dim=sequence_len, activation="relu"))
         
        discriminator.add(Dense(512, activation="relu"))
              
        discriminator.add(Dense(256, activation="relu"))
       
        discriminator.add(Dense(sequence_len, activation='tanh'))
        
        discriminator.compile(loss='binary_crossentropy', optimizer=Adam(learning_rate=0.001))
        return discriminator

    def _create_model(self, sequence_len):
        """Creates the GAN Model as described
        ...
        vae = self.create_vae(sequence_len)
        discriminator = self.create_discriminator(sequence_len)
        generator = self.create_generator(sequence_len)

        
        print("Sequence length is",sequence_len)
        gan_in = Input(shape=(sequence_len,))
        z = generator(gan_in)

        gan_out = discriminator(z)

        model = Model(gan_in,gan_out)
